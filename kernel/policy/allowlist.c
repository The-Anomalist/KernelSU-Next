    ...
    if (unlikely(!strcmp(profile->key, "$"))) {
...
#endif
    } else if (profile->curr_uid <= BITMAP_UID_MAX) {
        if (profile->allow_su)
            allow_list_bitmap[profile->curr_uid / BITS_PER_BYTE] |=
                1 << (profile->curr_uid % BITS_PER_BYTE);
        else
            allow_list_bitmap[profile->curr_uid / BITS_PER_BYTE] &=
                ~(1 << (profile->curr_uid % BITS_PER_BYTE));
    } else {
        if (profile->allow_su) {
            ...
            if (allow_list_pointer >= ARRAY_SIZE(allow_list_arr)) {
                ...
                allow_list_arr[allow_list_pointer++] = profile->curr_uid;
            }
        } else {
            remove_uid_from_arr(profile->curr_uid);
        }
    }
...
void ksu_load_allow_list(void)
{
...
#ifdef CONFIG_KSU_DEBUG
	/* Always allow adb shell by default in debug builds. */
	...
	if (ksu_kernel_read_compat(fp, &magic, sizeof(magic), &off) !=
		    sizeof(magic) ||
	    magic != FILE_MAGIC) {
		pr_err("allowlist file invalid: %u!\n", magic);
		goto exit;
	}

	if (ksu_kernel_read_compat(fp, &version, sizeof(version), &off) !=
	    sizeof(version)) {
		pr_err("allowlist read version: %u failed\n", version);
		goto exit;
	}

	if (version < 2 || version > KSU_APP_PROFILE_VER) {
		pr_err("invalid allowlist version: %u\n", version);
		goto exit;
	}

	pr_info("allowlist version: %u\n", version);
	...
	while (true) {
		...
		pr_info(
			"load_allow_uid, name: %s, uid: %d, allow: %d\n",
			profile.key,
			profile.curr_uid,
			profile.allow_su
		);
